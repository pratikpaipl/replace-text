...
@Component({
  selector: 'add-investee',
  templateUrl: 'add-investee.page.html',
  styleUrls: ['add-investee.page.scss'],
})


export class AddInvestee implements OnInit {
  ;
  number1:any
  ...
  isProvider = false
  isSizeExclude = false
  isSizeExcludeTeaser = false
  isLogoSizeExclude = false
  sizeExcludeErr = ''
  sizeExcludeErrTeaser = ''
  sizeLogoExcludeErr = ''
  ...
  representingFor:any =true;
  // desclaimerShow=true;
  // confidentialityShow=true;
  additionalDatas:any =true;
  ...
  getData(type) {
    this.subscription = this.apiService.getData('investee', (type =='add-with-enabler'?'add':type), (type=='edit_fn'|| type=='add_fn'|| type =='add-with-enabler'?'FundraiseSlug':type=='add'?'EnablerSlug':'FN_EnablerSlug'), (this.FN_EnablerSlug != undefined && this.FN_EnablerSlug != ''?this.FN_EnablerSlug:''),'EnablerSlug',this.EnablerSlug).subscribe(async response => {
      let res: any = response;
      if (res.success && res.data != undefined) {
        ...
        if(this.enablerData != undefined){
          ...
          this.selectedEnabler = this.enablerData.EnablerID;
        }
        this.isView = true
        // setTimeout(() => {
        //   $('#IsDisclaimer').first().focus();
        ...
        if(res.data.incorporation_country_list != undefined && res.data.incorporation_country_list.length > 0 ){
          this.incorporation_country_list =res.data.incorporation_country_list
        }
  ...
  async callAction(event, type, isFirst?) {
    ...
    let isCall = false
    if (isFirst && event == 1) {
      if((!this.store.isLogin) && (this.CompanyName == undefined || this.CompanyName == '')) {
      ...
      if (isCall) {
        ...
        if(this.store.isLogin){
          ...
          if(check == -1 ){
            postData.append("CompanyName", this.existCompany);
            postData.append("CompanyID", '');
          }else{
            postData.append("CompanyName", this.CompanyList[check].CompanyName);
      ...
      if (!this.ConfirmInvestee && this.enablerData != undefined) {
        this.ConfirmInvesteeUn = true;
        isCall = true
      }
      ...
      if (isCall) {
        ...
        postData.append("ConfirmInvestee", this.ConfirmInvestee ? '1' : '0');
        if(this.pdfFile !=undefined)
        postData.append("Pitch", this.pdfFile, this.pdfFile.name);
        if(this.teaserFile !=undefined)
        postData.append("Teaser", this.teaserFile, this.teaserFile.name);
        // if(this.IsDeletePitch)
        postData.append("IsDeleteTeaser", this.IsDeleteTeaser ? '1' : '0');
        ...
        this.apiService.addData('fundraise', postData, (this.type == 'clone'||this.type =='add-with-enabler')?'add':this.type).subscribe(async response => {
          ...
          if (res.success) {
            ...
            this.isSizeExcludeTeaser = false
            this.isLogoSizeExclude = false
            this.loadMsg = ''
            ...
            if( this.type =='add-with-enabler'){
              ...
              if(res.data.FundraiseSlug != undefined && this.selectedEnabler == undefined){
                if(this.store.isLogin){
    ...
    selectDataStatus(event) {
     ...
     setDetailsData(res: any) {
      if (res.data != undefined && res.data.detail != undefined) {
        ...
        this.ShortDescription = (detail.ShortDescription != undefined && detail.ShortDescription != '') ?  this.store.nl2br2(detail.ShortDescription) : ''
        ...
        this.ImpactSustainabilityStatement = detail.ImpactSustainabilityStatement !=undefined?this.store.nl2br2(detail.ImpactSustainabilityStatement) :this.ImpactSustainabilityStatement
        ...
        if(detail.Teaser != undefined && detail.Teaser !=''){
          this.fileUrlTeaser = detail.Teaser
        }
        ...
        if(detail.TeaserOriginalName != undefined && detail.TeaserOriginalName !=''){
          this.fileUrlTeaser = detail.TeaserOriginalName
        }
        ...
        this.ConfirmInvestee = detail.ConfirmInvestee == 1
    ...
    changeListener($event,type?): void {
      if ($event.target.files && $event.target.files[0]) {
        ...
        if(type == 'logo') {
          this.sizeLogoExcludeErr = ''
          this.isLogoSizeExclude=false
          if (files.size > this.store.logo_max_size) {
            this.isLogoSizeExclude=true
            this.store.scrollTo('upload');
            this.sizeLogoExcludeErr = this.store.getVal('maximum_size_allowed_is')+' ' + this.store.logo_max_size / 1000000 + this.store.getVal('mb');
          } else {
          ...
          if (files.size > this.store.max_size) {
            this.isSizeExcludeTeaser=true
            this.store.scrollTo('teaserFile');
            this.sizeExcludeErrTeaser = this.store.getVal('maximum_size_allowed_is')+' ' + this.store.max_size / 1000000 + this.store.getVal('mb');
          } else {
            this.teaserFile = files;
            this.fileNameTeaser = files.name
            let reader = new FileReader();
            reader.onload = (event: any) => {
              // this.img1 = event.target.result;
            }
            reader.readAsDataURL(this.teaserFile);
          }
        }else{
          this.sizeExcludeErr = ''
          this.isSizeExclude=false
    ...
    onDeleteTeaser(){
      this.IsDeleteTeaser=true
      this.fileUrlTeaser=undefined
      this.teaserFile = undefined;
      this.isSizeExcludeTeaser = false
      this.fileNameTeaser=''
      $('#teaserUpload').val('')
      console.log('1577')
 ...
 async openDesModal(fronType,from){
  ...
  if(this.selectedEnabler == undefined){
    // this.router.navigateByUrl(((this.store.appType =='admin')?'admin/':'')+'/investee/'+this.type+'/'+fronType);
    let path = ((this.store.appType =='admin')?'admin/':'')+'investee/'+this.type+'/'+fronType
    console.log('path ',path)
    this.router.navigateByUrl(path);
  }
  else{
    let path = ((this.store.appType =='admin')?'admin/':'')+'enabler/'+this.EnablerSlug+'/investee/'+this.type+'/'+fronType
    console.log('path ',path)
    this.router.navigateByUrl(path);
  }
  }
  ...
  setDatas(res: any, details: any) {
    ...
    this.confidentiality = res.data.confidentiality != undefined?res.data.confidentiality:this.store.getVal('investee_confidentiality_text');
    this.disclaimer = res.data.disclaimer!=undefined?res.data.disclaimer:this.store.getVal('investee_disclaimer_text')
    ...
    if (res.data.funding_type_list
      != undefined && res.data.funding_type_list
      .length > 0) {
      this.FundingTypeAndFundRaiseTypeList = res.data.funding_type_list;
    }
