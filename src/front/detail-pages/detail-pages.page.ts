...
@Component({
  selector: 'detail-pages-page',
  templateUrl: 'detail-pages.page.html',
  styleUrls: ['detail-pages.page.scss'],
  queries: {
    content: new ViewChild('content')
  }
})
export class DetailPages implements OnInit, OnDestroy, AfterContentChecked  {
  ...
  constructor(@Inject(PLATFORM_ID) platformId: any,private cookieService: CookieService, public myElement: ElementRef, public store: StorageService,  private route: ActivatedRoute, private activatedRoute: ActivatedRoute, private eventService: EventService, public toastController: ToastController, public actionSheetController: ActionSheetController, public modalController: ModalController, public router: Router, public platform: Platform, public apiService: ApiService, private cdref: ChangeDetectorRef) {
    ...
    this.type = (this.from !== 'user') ? urlParts[1] : urlParts[2];
  ...
  async getDetails() {
    try {
      const response = await this.apiService.getFrontDetails(this.from, this.type, this.slug, '', this.sort,(this.from == 'user' && this.type =='enabler'?'1':undefined),(this.from == 'user'&& this.type =='enabler'?'1':undefined)).toPromise();
      ...
      if (res.success && res.data.detail !== undefined) {
        ...
        if (this.type === 'enabler') {
          this.slug = this.detailData.EnablerSlug;
          this.byName = this.Name = this.detailData.EnablerName;
          this.parentDesc = this.detailData.ShortDescription;
          this.parentLogo = this.detailData.EnablerLogo;
          this.EnablerIDs = this.detailData.EnablerID;
          this.FNImpactThemesAndAreas = this.parseJson(this.detailData.FNImpactThemesAndAreas);
          this.FNEsg = this.parseJson(this.detailData.FNEsg);
          this.FPImpactThemesAndAreas = this.parseJson(this.detailData.FPImpactThemesAndAreas);
          this.FPEsg = this.parseJson(this.detailData.FPEsg);
          this.FNFundingType = this.parseJson(this.detailData.FNFundingType);
          this.FPFundingType = this.parseJson(this.detailData.FPFundingType);
          this.FNSdgs = this.parseJson(this.detailData.FNSdgs);
          this.FPSdgs = this.parseJson(this.detailData.FPSdgs);
        } else if (['fundraise-needs', 'fundraise-need', 'fundraise'].includes(this.type)) {
        ...
        if (this.segments.length === 0) {
          if (this.type === 'enabler') {
            ...
            // }
            if (this.detailData.IsOperative !== '0') {
              this.segments.push({ key: 'contact', label: this.store.getVal('contact'), icon: 'fa-comment fa-regular' });
            }
          }
          ...
          if (this.type === 'impact-funding-resources') {
            this.segments.push({ key: 'overview', label: this.store.getVal('overview'), icon: 'fa-solid fa-vector-square' });
          }
        }
        if (this.PLATFORMID && window.location.pathname.includes('user/') && this.type !== 'enabler') {
          this.segments.push({ key: 'matching-enablers', label: this.store.getVal('matching-enablers'), icon: 'fa-solid fa-vector-square' });
        }
        setTimeout(() => {
