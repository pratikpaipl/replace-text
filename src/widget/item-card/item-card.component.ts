...
export class ItemCardComponent implements OnInit {
  ...
  cutomData(type){
    return type == 'profile'?{ProfileName:this.prod.ProfileName,ProfileSlug:this.prod.ProfileSlug,ReferenceCode:this.prod.ReferenceCode}:{CompanyName:this.prod.CompanyName,FundraiseSlug:this.prod.FundraiseSlug,ReferenceCode:this.prod.ReferenceCode}
  }
  checkShowMore(item) {
  ...
  teaserPath(){
    return (this.from =='front'?'marketplace':this.from)+'/investee/'+(this.prod.FundraiseSlug != undefined?this.prod.FundraiseSlug:this.prod.FN_EnablerSlug !=undefined?this.prod.FN_EnablerSlug:this.FNEnablerSlug)+'/teaser'
  }
  openClick(prod) {
    prod.activePage = ''
      ...
      if (type == 'funding-profiles' && this.prod.FP_EnablerSlug != undefined && this.from == 'profile' && this.pageFrom != 'details') {
        ...
        if(this.from =='user'){
          if(this.prod.IsMyFundingProfile == 1|| this.prod.IsMyFundraise == 1) {
            strPath = ('/user/'+(this.type =='funding-profiles'?'funding-profile':'fundraise-need')+'/submissions/' +( (this.prod.FN_EnablerSlug !=undefined?this.prod.FN_EnablerSlug:this.prod.FP_EnablerSlug))+this.store.getSeprater('user',apPath)+(this.type =='funding-profiles'?'profile-form':'fundraise-form'))
          }else if(this.pageFrom =='details'){
            strPath = ('/user/'+this.prod.EnablerSlug+'/submitted/'+(this.type =='funding-profiles'?'funding-profile':'fundraise-need')+'/' +( (this.prod.FN_EnablerSlug !=undefined?this.prod.FN_EnablerSlug:this.prod.FP_EnablerSlug))+this.store.getSeprater('user',apPath)+(this.type =='funding-profiles'?'profile-form':'fundraise-form'))
          }
          else{
